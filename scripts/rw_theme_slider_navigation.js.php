...
<script>
    var SLWIDTHR = jQuery('.SLWIDTHR_<?php echo esc_html($rw_slider_image_id);?>').val();
    var SLHEIGHTR = jQuery('.SLHEIGHTR_<?php echo esc_html($rw_slider_image_id);?>').val();
    jQuery('#RW_Load_Content_Navigation<?php echo esc_html($rw_slider_image_id);?>').css('max-height', jQuery(
        '#RW_Load_Content_Navigation<?php echo esc_html($rw_slider_image_id);?>').width() * SLHEIGHTR / SLWIDTHR);
    jQuery(window).resize(function() {
        jQuery('#RW_Load_Content_Navigation<?php echo esc_html($rw_slider_image_id);?>').css('max-height', jQuery(
            '#RW_Load_Content_Navigation<?php echo esc_html($rw_slider_image_id);?>').width() * SLHEIGHTR / SLWIDTHR);
    })
    var array_nav<?php echo esc_html($rw_slider_image_id);?> = [];
    jQuery(".IMHR<?php echo esc_html($rw_slider_image_id);?>").each(function() {
        if (jQuery(this).attr("src") != "") {
            array_nav<?php echo esc_html($rw_slider_image_id);?>.push(jQuery(this).attr("src"));
        }
    })
    var y_nav<?php echo esc_html($rw_slider_image_id);?> = 0;
    for (i = 0; i < array_nav<?php echo esc_html($rw_slider_image_id);?>.length; i++) {
        jQuery("<img class='IMHR<?php echo esc_html($rw_slider_image_id);?>' />").attr('src', array_nav<?php echo esc_html($rw_slider_image_id);?>[i])
            .on("load", function() {
                y_nav<?php echo esc_html($rw_slider_image_id);?>++;
                if (y_nav<?php echo esc_html($rw_slider_image_id);?> == array_nav<?php echo esc_html($rw_slider_image_id);?>.length) {
                    jQuery("#rich_webSlider1_<?php echo esc_html($rw_slider_image_id);?>").css("display", "block");
                    jQuery("#RW_Load_Content_Navigation<?php echo esc_html($rw_slider_image_id);?>").remove();
                    jQuery('#rich_webSlider1_<?php echo esc_html($rw_slider_image_id);?> .flexslider').flexslider({
                        slideshow: <?php echo esc_html($rw_theme_options_arr->rich_web_Sl1_SlS);?>,
                        slideshowSpeed: <?php echo esc_html($rw_theme_options_arr->rich_web_Sl1_SlSS)*1000;?>,
                        pauseOnHover: <?php echo esc_html($rw_theme_options_arr->rich_web_Sl1_PoH);?>,
                        start: renderPreview, //render preview on start
                        before: renderPreview //render preview before moving to the next slide
                    });
                    var y<?php echo esc_html($rw_slider_image_id);?> = true;
                    function renderPreview(slider) {
                        ...
                        resp();
                        var delIc<?php echo esc_html($rw_slider_image_id);?> = document.querySelector(
                            ".delIc<?php echo esc_html($rw_slider_image_id);?>");
                        delIc<?php echo esc_html($rw_slider_image_id);?>.onclick = delVideo<?php echo esc_html($rw_slider_image_id);?>;
                        function delVideo<?php echo esc_html($rw_slider_image_id);?>() {
                            document.querySelector('.flexslider_<?php echo esc_html($rw_slider_image_id);?> ol').style.display =
                                "block";
                            document.querySelector('.flex-direction-nav').style.display = "block";
                            var videos = document.querySelectorAll('.rw_nav_video<?php echo esc_html($rw_slider_image_id);?>');
                            for (var j = 0; j < videos.length; j++) {
                                videos[j].setAttribute('src', '');
                                videos[j].classList.remove("rw_nav_video<?php echo esc_html($rw_slider_image_id);?>_anim");
                            }
                            delIc<?php echo esc_html($rw_slider_image_id);?>.style.display = "none";
                        }
                        prevWrapper.click(function() {
                            delVideo<?php echo esc_html($rw_slider_image_id);?>()
                        })
                    ...
                    function resp() {
                        var SLWIDTHR = jQuery('.SLWIDTHR_<?php echo esc_html($rw_slider_image_id);?>').val();
                        var SLHEIGHTR = jQuery('.SLHEIGHTR_<?php echo esc_html($rw_slider_image_id);?>').val();
                        var SLCLWR = jQuery('.SLCLWR_<?php echo esc_html($rw_slider_image_id);?>').val();
                        var SlClPrFS = jQuery('.SlClPrFS_<?php echo esc_html($rw_slider_image_id);?>').val();
                        var hovEffType = jQuery('.hovEffType_<?php echo esc_html($rw_slider_image_id);?>').val();
                        var navWidth = jQuery('.navWidth_<?php echo esc_html($rw_slider_image_id);?>').val();
                        var navHeight = jQuery('.navHeight_<?php echo esc_html($rw_slider_image_id);?>').val();
                        var navType = jQuery('.navType_<?php echo esc_html($rw_slider_image_id);?>').val();
                        jQuery(
                                '.flexslider_<?php echo esc_html($rw_slider_image_id);?>,#RW_Load_Content<?php echo esc_html($rw_slider_image_id);?>')
                            .css('height', Math.floor(jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() *
                                SLHEIGHTR / SLWIDTHR));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-next').css(
                            'height', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                .width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-prev').css(
                            'height', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                .width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-next').css('width',
                            Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-prev').css('width',
                            Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav').css('height', Math
                            .floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav a .preview').css('width',
                            Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .arrow').css('width',
                            Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-next .preview img')
                            .css('width', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                .width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-prev .preview img')
                            .css('width', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                .width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-control-paging li a').css('width', Math
                            .floor(navWidth * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() / 700));
                        jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-control-paging li a').css('height',
                            Math.floor(navHeight * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() /
                                700));
                        if (navType == 'bottom') {
                            jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-control-nav li').css('margin-top',
                        ...
                        if (hovEffType == 'slide out') {
                            jQuery(
                                ...
                                .css('right', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                    .width() / 700) + 'px');
                            jQuery(
                                    '.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-prev .preview img')
                                .css('left', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                    .width() / 700) + 'px');
                            jQuery(
                                ...
                                .css('right', Math.floor(2 * SLCLWR * jQuery(
                                    '.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() / 700) - 1 + 'px');
                            jQuery(
                                ...
                                .css('left', Math.floor(2 * SLCLWR * jQuery(
                                    '.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() / 700) - 2 + 'px');
                        } else if (hovEffType == 'flip out' || hovEffType == 'double flip out') {
                            jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-next .preview')
                                .css('right', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                    .width() / 700));
                            jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-prev .preview')
                                .css('left', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                    .width() / 700));
                            jQuery(
                                    '.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-next .preview .alt')
                        ...
                        jQuery(
                                '.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-next .preview .alt')
                            .css('width', Math.floor(2 * SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                .width() / 700) + 'px');
                        jQuery(
                                '.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-prev .preview .alt')
                            .css('width', Math.floor(2 * SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                        ...
                        jQuery(
                                '.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-next .preview .alt')
                            .css('font-size', Math.floor(SlClPrFS * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                .width() / 700) + 'px');
                        jQuery(
                                '.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-prev .preview .alt')
                            .css('font-size', Math.floor(SlClPrFS * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                        ...
                        jQuery(
                                '.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-next .preview .alt')
                            .css('line-height', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                .width() / 700) + 'px');
                        jQuery(
                                '.flexslider_<?php echo esc_html($rw_slider_image_id);?> .flex-direction-nav .flex-prev .preview .alt')
                            .css('line-height', Math.floor(SLCLWR * jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>')
                                .width() / 700) + 'px');
                        if (jQuery('.flexslider_<?php echo esc_html($rw_slider_image_id);?>').width() <= 400) {
...
<script type="text/javascript">
    ...
    function rw_nav_video_cl<?php echo esc_html($rw_slider_image_id);?>(src, el) {
        el.children[1].setAttribute('src', src + '?rel=0&amp;autoplay=1');
        el.children[1].classList.add("rw_nav_video<?php echo esc_html($rw_slider_image_id);?>_anim");
        document.querySelector('.flexslider_<?php echo esc_html($rw_slider_image_id);?> ol').style.display = "none";
        document.querySelector('.flex-direction-nav').style.display = "none";
        setTimeout(function() {
            document.querySelector('.delIc<?php echo esc_html($rw_slider_image_id);?>').style.display = "inline-block";
        }, 1000)
    }
    var delIc<?php echo esc_html($rw_slider_image_id);?> = document.querySelector(".delIc<?php echo esc_html($rw_slider_image_id);?>");
    delIc<?php echo esc_html($rw_slider_image_id);?>.onclick = delVideo<?php echo esc_html($rw_slider_image_id);?>;
    function delVideo<?php echo esc_html($rw_slider_image_id);?>() {
        document.querySelector('.flexslider_<?php echo esc_html($rw_slider_image_id);?> ol').style.display = "block";
        document.querySelector('.flex-direction-nav').style.display = "block";
        var videos = document.querySelectorAll('.rw_nav_video<?php echo esc_html($rw_slider_image_id);?>');
        for (var j = 0; j < videos.length; j++) {
            videos[j].setAttribute('src', '');
