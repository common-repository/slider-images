<script type="text/javascript">
	jQuery(document).ready(function() {
		var array_flexible<?php echo esc_html($rw_slider_image_id);?> = [];
		jQuery(".rw_fl_slider_img<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>").each(function() {
			if (jQuery(this).attr("src") != "") {
				array_flexible<?php echo esc_html($rw_slider_image_id);?>.push(jQuery(this).attr("src"));
			}
		})
		var y_flexible<?php echo esc_html($rw_slider_image_id);?> = 0;
		var Rich_Web_Sl_FS_Arr_Type<?php echo esc_html($rw_slider_image_manager_arr["id"]);?> = jQuery(
			".Rich_Web_Sl_FS_Arr_Type<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>").val();
		var answer = true;
		if (Rich_Web_Sl_FS_Arr_Type<?php echo esc_html($rw_slider_image_manager_arr["id"]);?> == "none") {
			answer = false;
		}
		for (i = 0; i < array_flexible<?php echo esc_html($rw_slider_image_id);?>.length; i++) {
			jQuery("<img class='RW_Fashion_IMG<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>' />").attr('src',
				array_flexible<?php echo esc_html($rw_slider_image_id);?>[i]).on("load", function() {
				y_flexible<?php echo esc_html($rw_slider_image_id);?>++;
				if (y_flexible<?php echo esc_html($rw_slider_image_id);?> == array_flexible<?php echo esc_html($rw_slider_image_id);?>
					.length) {
					var slider = jQuery('.mis-stage_<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>')
						.miSlider({
							speed: <?php echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_TS);?>,
							pause: <?php if($rw_theme_options_arr->Rich_Web_Sl_FS_AP=='true'){ echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_PT*1000);}else{ echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_AP);} ?>,
							increment: <?php echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_SS);?>,
							slidesOnStage: <?php echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_CS); ?>,
							slidesContinuous: <?php echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_SLoop);?>,
							slidePosition: "<?php echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_SlPos);?>",
							slideScaling: <?php echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_SSc);?>,
							navList: <?php echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_Nav_Show);?>,
							navButtons: answer,
							navButtonsShow: <?php echo esc_html($rw_theme_options_arr->Rich_Web_Sl_FS_ShNavBut);?>,
							classNavButtons: "mis-nav-buttons_<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>",
							classNavList: "mis-nav-list_<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>",
							classSlide: "mis-slide_<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>",
							classStage: "mis-stage_<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>",
							classSlider: "mis-slider_<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>",
						});
					jQuery(".rw_fl_slider_figure<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>").css(
						"height", "auto");
...
<script type="text/javascript">
	var Rich_Web_Sl_FS_SVis<?php echo esc_html($rw_slider_image_manager_arr["id"]);?> = document.querySelector(
		".Rich_Web_Sl_FS_SVis<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>").getAttribute('value');
	function resp(el) {
	...
	function createPopup(vSrc, iSrc) {
		...
		overlay.setAttribute("class", "flPopupOverlay<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>");
		document.body.appendChild(overlay);
		var content = document.createElement("div");
		content.setAttribute("class", "flPopupOverlayContent<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>");
		overlay.appendChild(content);
		var delIcon = document.createElement("div");
		delIcon.setAttribute("class", "flSlDelIcon<?php echo esc_html($rw_slider_image_manager_arr["id"]);?> rich_web rich_web-times");
		...
		if (vSrc != "") {
			...
			video.setAttribute("class", "flPopupVideo<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>");
		...
		setTimeout(function() {
			overlay.classList.add("flPopupOverlay<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>_anim");
			content.classList.add("flPopupOverlayContent<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>_anim");
		}, 100)
		setTimeout(function() {
			if (vSrc != "") {
		...
		function delPop() {
			...
			overlay.classList.remove("flPopupOverlay<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>_anim");
			content.classList.remove("flPopupOverlayContent<?php echo esc_html($rw_slider_image_manager_arr["id"]);?>_anim");
			setTimeout(function() {
				overlay.remove();
			}, 300)
		}
